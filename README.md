...
- 실험은 여러가지 요소를 변경해가며 진행했습니다.
	- 데이터셋, 임베딩 방법, 텍스트 처리 방법, 단어 최소 빈도수 threshold, 에포크(epoch), 배치사이즈(batch size), 학습률(learning rate) 
...
- 고유한 캡션들과 이미지들만 사용하여 학습
	- 동영상이라는건 결국 이미지들의 연속인데, 데이터 구조를 보면 동영상 안에는 여러 장면들이 있고 하나의 동일한 장면에 대해선 프레임 이미지들이 다 비슷합니다. 그렇기 때문에 캡션도 동일합니다.
